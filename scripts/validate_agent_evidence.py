#!/usr/bin/env python3
"""Validate AgentsWatch agent evidence rows and run logs.
...
MISTAKE_ID_RE = re.compile(r"\bAW-MISTAKE-[A-Z0-9]+(?:-[A-Z0-9]+)*\b")
...
def validate_run_log(path: Path, known_mistakes: set[str], referenced_logs: set[Path]) -> list[Finding]:
    ...
    if "## Mistakes observed" not in text:
        ...
        if "none" not in mistakes_section.casefold():
            ...
            if not mistake_ids:
                findings.append(Finding("FAIL" if strict else "WARN", path, 1, "Mistakes observed is not 'none' and has no AW-MISTAKE-* ID"))
...
def validate_all(strict_legacy: bool = False, all_run_logs: bool = True) -> list[Finding]:
    ...
    if not known_mistakes:
        findings.append(Finding("FAIL", MISTAKE_LEDGER, 1, "mistake ledger missing or contains no AW-MISTAKE-* IDs"))
